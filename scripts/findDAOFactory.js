...
async function main() {
  ...
  // List of all known factory addresses (old and new)
  const factories = [
  ...
  for (const factory of factories) {
    ...
    try {
      ...
      // Try to find NewDaoCreated event
      const filter = factoryContract.filters.NewDaoCreated();
      const events = await factoryContract.queryFilter(filter, -5000, "latest"); // Last 5000 blocks

      for (const event of events) {
        if (event.args.daoAddress.toLowerCase() === daoAddress.toLowerCase()) {
          console.log(`\n✓✓✓ FOUND! DAO was created by: ${factory.name}`);
          console.log(`    Factory address: ${factory.address}`);
          console.log(`    Block: ${event.blockNumber}`);
          console.log(`    Token from event: ${event.args.tokenAddress}`);
  ...
  console.log("\n✗ DAO not found in any known factory");
}
